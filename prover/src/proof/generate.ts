...
export function toFixed(val: number): bigint {
  return BigInt(Math.round(val * FIX_SCALE));
}

// ── Neural Network Inference (reference) ───────────────────────────

export function runInference(input: InferInput, weights: number[], outputWeights: number[]): InferenceResult {
  const raw = [
...
async function ensureCompiled(nargoBin?: string): Promise<void> {
  ...
  if (!existsSync(acirPath)) {
    ...
    const target = nargoBin || (
      existsSync(join(process.env.HOME || '', '.cargo/bin/nargo'))
...
export async function generateProof(
  ...
  const rawCircuit = JSON.parse(readFileSync(circuitPath, 'utf-8')) as {
    bytecode: string;
    abi?: any;
  };
  ...
  const proofData = { proof, publicInputs: publicInputs as string[] };
  const valid = await backend.verifyProof(proofData as any);
